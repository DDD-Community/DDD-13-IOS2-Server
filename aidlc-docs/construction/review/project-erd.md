...
erDiagram
    ...
    %% [FC-4] group_member — 그룹 구성원 (참석여부는 meeting_participant로 이전, V31에서 attendance_status 제거)
    group_member {
    ...
    %% [장소선정 신규] meeting_place_pick — 담기(+V28 source 백필구분)
    meeting_place_pick {
        BIGINT id PK
        BIGINT meeting_id FK
        BIGINT member_id FK "V28 nullable, SYSTEM은 NULL"
        BIGINT place_id FK
        VARCHAR source "V28 USER/SYSTEM"
